...
def verify_binary(cfg) -> str:
    ...
    if not cfg.xray_binary or not path.is_absolute() or not path.is_file():
        raise RuntimeUnavailable("VMess requires LUNEL_XRAY_BINARY (absolute installed Xray path)")
    expected = cfg.xray_sha256.lower()
    if len(expected) != 64 or any(c not in "0123456789abcdef" for c in expected):
        raise RuntimeUnavailable("VMess requires LUNEL_XRAY_SHA256 from a trusted release")
...
class XrayRuntime:
    ...
    def config(uuid: str, port: int) -> dict:
        ...
        return {
            ...
            "inbounds": [{
                "tag": "lunel-vmess", "listen": "127.0.0.1", "port": port,
                "protocol": "vmess",
    ...
    async def _start(self, uuid: str) -> dict:
        ...
        directory = tempfile.TemporaryDirectory(prefix="lunel-xray-")
        proc = None
        try:
