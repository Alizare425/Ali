...
async def export(request: Request, _=Depends(admin_user)):
    ...
    return Response(raw, media_type="application/json", headers={
        **HEADERS, "Content-Disposition": 'attachment; filename="lunel-configuration-backup-v1.json"',
    })
...
async def restore(request: Request, admin=Depends(admin_user)):
    if request.headers.get("x-lunel-backup-confirm") != "import":
        raise HTTPException(400, detail="Explicit confirmation required: X-Lunel-Backup-Confirm: import")
    doc = await document(request)
