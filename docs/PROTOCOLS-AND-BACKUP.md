...
Select `vmess-ws` in the wizard. Lunel provides a WebSocket gateway to a
...
- `LUNEL_XRAY_BINARY`: absolute path to an operator-installed Xray executable.
- `LUNEL_XRAY_SHA256`: 64-character expected SHA-256 digest, checked before launch.
- `LUNEL_XRAY_MAX_RUNTIMES`: maximum concurrent per-instance Xray processes
...
1. Use a separately prepared target whose `LUNEL_SECRET_KEY` matches the source.
