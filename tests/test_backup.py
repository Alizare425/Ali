...
import httpx
from lunel_console.db import _SqliteDatabase, SQLITE_SCHEMA
from lunel_console.services import backup
from lunel_console.routers import backup as router
from lunel_console.auth import sessions
...
class BackupTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_router_auth_csrf_confirmation_and_roundtrip(self):
        ...
        async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://test") as client:
            with patch("lunel_console.routers.admin.get_pool", return_value=self.db), patch.object(router, "get_pool", return_value=self.db), patch.object(sessions, "get_session_user", auth), patch.object(router.settings, "secret_key", SECRET):
                ...
                client.cookies.set("lunel_session", "synthetic-session")
                self.assertEqual((await client.post("/api/admin/backup/export")).status_code, 403)
                client.headers["X-Lunel-CSRF"] = "synthetic-session"
                ...
                response = await client.post("/api/admin/backup/restore", json=self.doc, headers={"X-Lunel-Backup-Confirm": "import"})
