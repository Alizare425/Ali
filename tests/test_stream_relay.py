...
from lunel_core.app import Core
from lunel_core.config import CoreConfig
from lunel_core.state import Link
from lunel_core.relay.trojan import build_trojan_request
...
async def _stream_up_through_relay():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        os.environ["LUNEL_WORKER_TOKEN"] = WORKER_TOKEN
        os.environ["LUNEL_WORKER_DATA"] = str(Path(tmp) / "data")
        from lunel_worker import main as worker_main
        ...
        from lunel_console.services import gateway as gateway_module
        ...
        from lunel_console import config as console_config

        console_config.settings.worker_token = WORKER_TOKEN
        gw = core_srv  # placeholder to keep names tidy
        from lunel_console.main import app as console_app
